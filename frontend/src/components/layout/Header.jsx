...
const tabs = [
  { label: 'Home', path: '/' },
  {
    label: 'Our Services', href: '#ServicesSection', isAnchor: true
  },
...
const Header = () => {
  ...
  const headerClasses = isHomePage
    ? 'absolute top-0 left-0 right-0 z-50 text-white py-8 px-4 flex items-center justify-between bg-transparent'
  ...
  <ul className="flex gap-6 text-sm font-semibold relative">
    {tabs.map((tab) => (
        ...
        {tab.subItems && (
          <ul className="absolute left-0 top-full mt-2 bg-white text-black shadow-xl rounded-md border border-gray-200 w-60 max-h-40 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100 opacity-0 group-hover:opacity-100 invisible group-hover:visible transition-all duration-300 z-50">
            {tab.subItems.map((item) => (
              <li key={item.label} className="border-b last:border-b-0 border-gray-100">
                <Link
                  to={item.path}
